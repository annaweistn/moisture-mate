...
const amqp_message_1 = require("./amqp_message");
const errors = require("../common-core/errors");
const results = require("../common-core/results");
const amqp_cbs_1 = require("./amqp_cbs");
const sender_link_1 = require("./sender_link");
const receiver_link_1 = require("./receiver_link");
...
class Amqp {
    ...
    constructor(autoSettleMessages) {
        ...
        this._rheaContainer = (0, rhea_1.create_container)();
        this._rheaContainer.on('azure-iot-amqp-base:error-indicated', (err) => {
            debugErrors('azure-iot-amqp-base:error-indicated invoked ' + (0, amqp_common_errors_1.getErrorName)(err));
            this._fsm.handle('amqpError', err);
        });
        const rheaErrorHandler = (context) => {
        ...
        const manageConnectionHandlers = (operation) => {
        ...
        const sessionErrorHandler = (context) => {
            debugErrors(`session error event handler: error = ${(0, amqp_common_errors_1.getErrorName)(context.error)}`);
            this._fsm.handle('session_error', context);
        };
        const sessionOpenHandler = (context) => {
            debug('session open event handler');
            this._fsm.handle('session_open', context);
        };
        const sessionCloseHandler = (context) => {
            debug('session close event handler');
            this._fsm.handle('session_close', context);
        };
        const manageSessionHandlers = (operation) => {
            ...
            states: {
                disconnected: {
                    _onEnter: (disconnectCallback, err, _result) => {
                        if (disconnectCallback) {
                            ...
                            debugErrors('error passed to disconnect handler is: ' + (0, amqp_common_errors_1.getErrorName)(err || new errors.NotConnectedError('rhea: connection disconnected')));
                ...
                connected: {
                    ...
                    initializeCBS: (callback) => {
                        this._cbs = new amqp_cbs_1.ClaimsBasedSecurityAgent(this._rheaSession);
                    ...
                    send: (message, endpoint, to, done) => {
                        ...
                        const amqpMessage = amqp_message_1.AmqpMessage.fromMessage(message);
                        if (to !== undefined) {
                            amqpMessage.to = to;
                        }
                        if (!this._senders[endpoint]) {
                            this._fsm.handle('attachSenderLink', endpoint, null, (err) => {
                                if (err) {
                                    debugErrors('failed to attach the sender link: ' + (0, amqp_common_errors_1.getErrorName)(err));
                                    done(err);
                                }
                                else {
                                    this._senders[endpoint].send(amqpMessage, done);
                                }
                            });
                        }
                        else {
                    ...
                    attachReceiverLink: (endpoint, linkOptions, done) => {
                        debug('creating receiver link for: ' + endpoint);
                        this._receivers[endpoint] = new receiver_link_1.ReceiverLink(endpoint, linkOptions, this._rheaSession);
                    ...
                    attachSenderLink: (endpoint, linkOptions, done) => {
                        debug('creating sender link for: ' + endpoint);
                        this._senders[endpoint] = new sender_link_1.SenderLink(endpoint, linkOptions, this._rheaSession);
                        const permanentErrorHandler = (_err) => {
                    ...
                    detachReceiverLink: (endpoint, detachCallback) => {
                        if (!this._receivers[endpoint]) {
                    ...
                    detachSenderLink: (endpoint, detachCallback) => {
                ...
                disconnecting: {
                    _onEnter: (disconnectCallback, err) => {
                        debug('Entering disconnecting state with disconnectCallback: ' + disconnectCallback + ' error of: ' + (0, amqp_common_errors_1.getErrorName)(err));
                        ...
                        const disconnect = (callback) => {
                            ...
                            if (err) {
                                debugErrors('with a disconnecting state err: ' + (0, amqp_common_errors_1.getErrorName)(err));
                            }
                            //
                        ...
                        //
                        let rerunWithForceTimer = undefined;
                        const rundownConnection = (force) => {
                            //
                    ...
                    amqpError: (err) => {
                        debugErrors('ignoring error event while disconnecting: ' + (0, amqp_common_errors_1.getErrorName)(err));
                    },
     ...
     */
    setDisconnectHandler(disconnectCallback) {
    ...
    _detachLink(link, detachCallback) {
        if (!link) {
